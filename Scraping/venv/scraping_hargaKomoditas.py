...
def array_data_komoditas_mean(tanggal):
  ...
  for row in rows:
    ...
    print('arr_komoditas',arr_komoditas)
...
def insert_or_get_pasar(id_pasar, nm_pasar, id_kabkota, nm_kabkota, keycode_kabkota):
  """Insert kabkota dan pasar jika belum ada"""
  # Insert kabupaten/kota jika belum ada
  insert_kabkota_query = """
  INSERT INTO kabupaten_kota (id, nama, keycode)
  VALUES (%s, %s, %s)
  ON CONFLICT (id) DO NOTHING
  """
  cursor.execute(insert_kabkota_query, (id_kabkota, nm_kabkota, keycode_kabkota))
  
  # Insert pasar jika belum ada
  insert_pasar_query = """
  INSERT INTO pasar (id, nama, kabupaten_kota_id)
  VALUES (%s, %s, %s)
  ON CONFLICT (id) DO NOTHING
  """
  cursor.execute(insert_pasar_query, (id_pasar, nm_pasar, id_kabkota))
...
VALUES (%s, %s, %s, %s, %s, %s)
ON CONFLICT (tanggal, pasar_id, kategori_id, komoditas_nama) DO NOTHING
"""
...
for tanggal in date_list:
  ...
  for my_kabkota in list_kabkota:
    ...
    for my_pasar in data_pasar:
      ...
      # Insert atau ambil pasar
      insert_or_get_pasar(id_pasar, nm_pasar, id_kabkota, nama_kabkota, my_kabkota)
